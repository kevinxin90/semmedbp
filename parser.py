...
def load_data(data_folder):
    ...
    with open(edges_path) as f:
        ...
        next(csv_reader)
        for _item in csv_reader:
            if _item[4] in group_by_semmantic_dict['biological_process_or_activity']:
                if _item[4] not in bp_related:
                    bp_related[_item[4]] = {'_id': _item[4][5:],
                                            'umls': _item[4][5:],
                ...
                if semantic_type not in bp_related[_item[4]][pred]:
                    bp_related[_item[4]][pred][semantic_type] = []
                assoc = _item[4] + pred + str(_item[1]) + _item[5]
                if assoc not in unique_assocs:
                    unique_assocs.add(assoc)
                    bp_related[_item[4]][pred][semantic_type].append({'pmid': _item[1].split(';'), 'umls': _item[5][5:]})
            elif _item[5] in group_by_semmantic_dict['biological_process_or_activity']:
                if _item[5] not in bp_related:
                    bp_related[_item[5]] = {'_id': _item[5][5:],
                                            'umls': _item[5][5:],
                ...
                if semantic_type not in bp_related[_item[5]][pred]:
                    bp_related[_item[5]][pred][semantic_type] = []
                assoc = _item[5] + pred + str(_item[1]) + _item[4]
                if assoc not in unique_assocs:
                    unique_assocs.add(assoc)
                    bp_related[_item[5]][pred][semantic_type].append({'pmid': _item[1].split(';'), 'umls': _item[4][5:]})
    for v in bp_related.values():
        yield v
